...
func (s *Session) SendTextToServer(data []byte) error {
	s.writeMu.Lock()
	defer s.writeMu.Unlock()
	return s.Server.WriteMessage(websocket.TextMessage, data)
}
func (s *Session) SendBinaryToServer(data []byte) error {
	s.writeMu.Lock()
	defer s.writeMu.Unlock()
	return s.Server.WriteMessage(websocket.BinaryMessage, data)
}
...
func (p *ProxyServer) handleWebSocket(w http.ResponseWriter, r *http.Request, isSecure bool) {
	...
	go func() {
		...
		for {
			...
			messageCtx := *ctx
			messageCtx.IsWebSocket = true
			messageCtx.MessageType = messageType
			for u, handle := range p.reqHandles {
				if u == All || strings.Contains(r.Host, u) {
					if handle != nil {
						modifiedMessage = handle(message, &messageCtx)
					}
				}
			}
			...
			if messageType == websocket.BinaryMessage {
				writeErr = ctx.WSSession.SendBinaryToServer(modifiedMessage)
			} else {
				writeErr = ctx.WSSession.SendTextToServer(modifiedMessage)
			}
			if writeErr != nil {
				p.logger.Error("Failed to send message to target server: %v", writeErr)
				return
			}
		}
	}()
